...
module.exports = ({ usuario, recovery }, cb) => {
  ...
  <br /><br /><hr />
  <p>
    Obs.: Se você não solicitou a redefinição, ignore esse e-mail
  </p>
  <br/>
  <p>Atenciosamente, Loja</p>
`;
  const opcoesEmail = {
    from: "naoresponder@loja.com.br",
    to: usuario.email,
  ...
  if (process.env.NODE_ENV === "production") {
    transporter.sendMail(opcoesEmail, (error, info) => {
      if (error) {
        ...
        return cb(
          null,
          "Link para redefinição de senha enviado para seu email. ",
        );
      }
    });
  } else {
